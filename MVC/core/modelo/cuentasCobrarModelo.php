...
//echo $sql;
    ...
    {   
		$sql = "update cuentasCobrar set plazo='".$datos[1]."',tipoPlazo='".$datos[2]."' where idventas='".$datos[0]."'";
//echo $sql;
			if($mysql->query($sql))
			{
...
function verCuentaC($dato)
{
    ...
    $mysql = conexionMysql();
    $form="";
    $sql = "SELECT cc.fecha,cc.plazo,cc.tipoPlazo,cc.creditodado,cc.total,cc.idventas,(select c.nombre from cliente c where c.idcliente=(select v.idcliente from ventas v where v.idventas=cc.idventas limit 1) limit 1),(select c.apellido from cliente c where c.idcliente=(select v.idcliente from ventas v where v.idventas=cc.idventas limit 1) limit 1),(select c.direccion from cliente c where c.idcliente=(select v.idcliente from ventas v where v.idventas=cc.idventas limit 1) limit 1),(select c.telefono from cliente c where c.idcliente=(select v.idcliente from ventas v where v.idventas=cc.idventas limit 1) limit 1) FROM cuentascobrar cc where cc.estado=1 and cc.idcuentasC='".$dato[0]."' ";
